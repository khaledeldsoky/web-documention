...
export function Section1() {
  return (
    <Section id="overview" num={1} title="Overview & Assumptions">
      <Prose>
        Topology: 3 masters + 3 workers (6 VMs total). <code>master1</code>{" "}
        doubles as the <strong>infra node</strong>: local DNS, local NTP, local
        yum repo, local container registry.
      </Prose>
      <Prose>
        No internet, no DHCP, no existing DNS, no NTP, no gateway, no IPs
        assigned yet.
      </Prose>
      ...
      <Subsection title="Air-Gap vs Normal Install">
        <InfoTable
          ...
          rows={[
            ...
            {
              normal: "DNS record on Windows DNS",
              airgap: "DNS record created in dnsmasq on master1 instead",
            },
          ]}
        />
      </Subsection>
